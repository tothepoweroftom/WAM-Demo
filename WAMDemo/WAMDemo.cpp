...
WAMDemo::WAMDemo(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new ITextControl(titleBounds, "WAMDemo", IText(30)), kCtrlTagTitle);
...
#if IPLUG_EDITOR
void WAMDemo::OnParentWindowResize(int width, int height)
{
  if(GetUI())
...
#if IPLUG_DSP
void WAMDemo::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{ 
