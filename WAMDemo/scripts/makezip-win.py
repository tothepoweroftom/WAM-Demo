...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\WAMDemo Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\WAMDemo Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\WAMDemo manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\WAMDemo.vst3\\Contents\\x86_64-win\\WAMDemo.vst3",
      projectpath + "\\build-win\\WAMDemo_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\WAMDemo-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\WAMDemo-app_x64.pdb"  
  ]
