/* Declares the WAMDemo Audio Worklet Node */

class WAMDemoController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "WAMDemo", options);
  }

  static importScripts (actx) {
    var origin = "https://tothepoweroftom.github.io/WAM-Demo/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/WAMDemo-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/WAMDemo-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
